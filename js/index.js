...
;(function () {
  ...
  var start ;
  var startTime
  imgUrl.addEventListener('touchstart',function (e) {
       // console.log('哈哈');
       // console.log(e);
    // 清除定时器
    clearInterval(timeId);
  ...
  //2 移除imgUrl 的动画
  // 重新设置imgUrl的位置
  imgUrl.addEventListener('touchmove',function (e) {
    // changedTouches 改变的手指
    // touches屏幕上所有的手指
    // targetTouches 在dom元素内的手指
  ...
  imgUrl.addEventListener('touchend',function (e) {
    // console.log('呵呵');
    var distance = e.changedTouches[0].clientX-start;
    ...
    console.log('时间:'+time);
    if(Math.abs(distance)>width/3 || (time<300 && Math.abs(distance)>30) ) {
       if(distance>0) {
         //向右滑动 到上一屏
          index--
       }
